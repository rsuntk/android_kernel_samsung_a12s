...
create_boot() { # make a flashable boot.img/tar, so we don't need custom AnyKernel3
	...
	if [[ $PERM_FLAGS = 'y' ]] && [[ $ENF_FLAGS = 'n' ]]; then
		SELINUX_STATE="Permissive"
	elif [[ $PERM_FLAGS = 'n' ]] && [[ $ENF_FLAGS = 'y' ]]; then
	...
	# Format
	if [[ $KSU = 'true' ]]; then
		KSU_VERSION=$(cd KernelSU && git describe --tags)
		BOOT_FMT="TragicHorizon-$(echo $SELINUX_STATE)-$(echo $KERN_REV)-KSU_$(echo $KSU_VERSION)-$(echo $KSU_BRANCH)_$(echo $GITSHA).img"
		TAR_FMT="TragicHorizon-$(echo $SELINUX_STATE)-$(echo $KERN_REV)-KSU_$(echo $KSU_VERSION)-$(echo $KSU_BRANCH)_$(echo $GITSHA).tar"
	else
		BOOT_FMT="TragicHorizon-$(echo $SELINUX_STATE)-$(echo $KERN_REV)_$(echo $GITSHA).img"
		TAR_FMT="TragicHorizon-$(echo $SELINUX_STATE)-$(echo $KERN_REV)_$(echo $GITSHA).tar"
	fi
